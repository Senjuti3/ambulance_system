...
export function AdminDashboard() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setErr("Could not load pending registrations (check admin token).");
    }
  }, []);
  ...
  return (
    <div className="space-y-10">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <h1 className="text-2xl font-bold text-white">Admin — verification queue</h1>
        <button
          type="button"
          onClick={() => void load()}
      ...
      <section>
        <h2 className="mb-3 text-lg font-semibold text-teal-300">Pending hospitals</h2>
        <div className="overflow-x-auto rounded-xl border border-slate-800">
          <table className="min-w-full divide-y divide-slate-800 text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-800">
              ...
              {hospitals.map((h) => (
                <tr key={h.public_id} className="bg-slate-950/40">
                  <td className="px-4 py-2 text-white">{h.name}</td>
                  <td className="px-4 py-2 text-slate-400">{h.email}</td>
                  <td className="px-4 py-2 text-slate-400">{h.uhid}</td>
                  <td className="px-4 py-2 text-slate-400">{h.hospital_type}</td>
                  <td className="px-4 py-2">
                    <input
                      className="w-40 rounded border border-slate-700 bg-slate-900 px-2 py-1 text-xs"
      ...
      <section>
        <h2 className="mb-3 text-lg font-semibold text-teal-300">Pending ambulances</h2>
        <div className="overflow-x-auto rounded-xl border border-slate-800">
          <table className="min-w-full divide-y divide-slate-800 text-left text-sm">
            <thead className="bg-slate-900/80 text-slate-400">
              <tr>
            ...
            <tbody>
              ...
              {ambulances.map((a) => (
                <tr key={a.ambulance_id} className="bg-slate-950/40">
                  <td className="px-4 py-2 font-mono text-teal-300">{a.ambulance_id}</td>
                  <td className="px-4 py-2 text-white">{a.driver_name}</td>
                  <td className="px-4 py-2 text-slate-400">{a.email}</td>
                  <td className="px-4 py-2 text-slate-400">{a.ambulance_type}</td>
                  <td className="px-4 py-2 text-slate-400">{a.vehicle_number}</td>
                  <td className="px-4 py-2">
                    <input
                      className="w-40 rounded border border-slate-700 bg-slate-900 px-2 py-1 text-xs"
                      placeholder="Admin note"
