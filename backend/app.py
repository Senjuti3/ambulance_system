...
from config import FRONTEND_ORIGIN, PORT, SECRET_KEY, UPLOAD_DIR
...
CORS(
    app,
    origins=[FRONTEND_ORIGIN, "http://localhost:3000", "http://127.0.0.1:5173"],
...
def _login_entity(email: str, password: str) -> tuple[dict, str, str] | None:
    ...
    admin = get_admin_by_email(email_l)
    if admin and verify_password(password, admin["password_hash"]):
        profile = {"name": admin.get("name"), "email": admin["email"]}
...
def api_login():
    ...
    token = create_token(subject_id, role, email.lower().strip())
...
def api_bootstrap_admin():
    ...
    create_admin_user(email, hash_password(password), name)
...
def api_hospital_register():
    ...
    if get_hospital_by_email(str(data["email"]).lower().strip()):
...
def api_ambulance_register():
    ...
    if get_ambulance_by_email(data["email"].lower().strip()):
        return jsonify({"error": "email already registered"}), 409
...
def api_cases():
    return jsonify({"cases": list_cases_recent(80)})
...
def init_app():
    ...
    try:
        ...
        if adm and pw and admin_count() == 0:
            create_admin_user(adm.lower().strip(), hash_password(pw), "Seed Admin")
